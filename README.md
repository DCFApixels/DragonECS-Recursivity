...
Simplifies event handling by stimulating recursive behavior, but works within the standard ECS flow. It guarantees that events, which need to be processed within a single frame, will be handled by all systems exactly once, regardless of where they originate.
